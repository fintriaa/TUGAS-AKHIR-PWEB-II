...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            ...
            @endif
            <button wire:click="create()" class="bg-orange-500 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded my-3">Masukkan Pemasukan</button>
            @if($isOpen)
                @include('livewire.create-masuk')
            @endif
            ...
            <h3 class="bg-orange-500 hover:bg-orange-500 text-white font-bold py-2 px-4 rounded my-5">Total Uang Masuk : {{ isset($post) ? $post->sum('uang') : 0 }} </h3>
        </div>
    </div>
</div>